...
function renderOverview() {
  ...
  chR = new Chart(document.getElementById('chartReceita'), {
    type: 'line',
    data: { labels: lb, datasets: [{ label: 'Receita', data: rv, borderColor: '#2d5a3d', backgroundColor: 'rgba(45, 90, 61,.1)', tension: .4, fill: true, pointBackgroundColor: '#2d5a3d' }] },
  ...
  chS = new Chart(document.getElementById('chartStatus'), {
    type: 'doughnut',
    data: { labels: ['Concluídos', 'Em andamento', 'Pendentes'], datasets: [{ data: [(sc.concluido||0)+(sc.recolhida||0), (sc['a-caminho']||0)+(sc.entregue||0), sc.pendente||0], backgroundColor: ['#6c757d', '#2d5a3d', '#f59e0b'], borderWidth: 0 }] },
...
function renderRelatorio() {
  ...
  chP = new Chart(document.getElementById('chartPedMes'), {
    type: 'bar',
    data: { labels: lb, datasets: [{ label: 'Pedidos', data: pv, backgroundColor: '#2d5a3d', borderRadius: 6 }] },
    options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
  });
  if (chA) chA.destroy();
  chA = new Chart(document.getElementById('chartRecAcum'), {
    type: 'line',
    data: { labels: lb, datasets: [{ label: 'Receita', data: av, borderColor: '#2d5a3d', backgroundColor: 'rgba(45, 90, 61,.08)', tension: .4, fill: true }] },
